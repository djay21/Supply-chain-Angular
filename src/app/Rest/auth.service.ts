...
export class AuthService {
  ...
  peers = 'peer0.{{orgName}}.truesource.com';
  ...
  signIn(username: string, orgName: string) {
    const body = new HttpParams()
      .set('username', username)
      .set('orgName', orgName);

    return this.http.post(this.baseUrl+'/users',body.toString(),
      {
      ...
      (data: any) => {
        ...
        this.getUserDetails(username,orgName);
      },
      error => {
  ...
  getUserDetails(username: string, orgName:string) {
    const body = new HttpParams()
      .set('username', username)
      .set('orgName', orgName);
  ...
  getUserHistory(username: string, orgName: string):any {
